...
class User extends CI_Model{
	...
	function update_user($user_id, $info)
	{
		...
		$query = "UPDATE users SET name = ?, alias = ?, email = ?, description = ?, user_level = ?, password = ?, updated_at = NOW() WHERE id = ?";
		$values = array($userInfo['name'], $userInfo['alias'], $userInfo['email'], $userInfo['description'], $userInfo['user_level'], $encrypted_password, $userInfo['id']);
		return $this->db->query($query, $values);
	}

	function validate($post){
		$this->form_validation->set_rules('name', 'Full Name', 'trim|max_length[45]|required');
